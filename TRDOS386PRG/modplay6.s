; ****************************************************************************
; modplay6.s (for TRDOS 386)
; ----------------------------------------------------------------------------
; MODPLAY6.PRG ! AC'97 (ICH) MOD PLAYER & VGA DEMO program by Erdogan TAN
;
; 24/06/2017
;
...
MakeOfs:        
	...
	;
	;19/10/2017
	;Note: AC'97 hardware plays music as stereo and 16 bits.
	;    'stmo' and 'bps' will not be recognized by TDDOS 386 kernel ('audio.s'). 	
	;
	mov	al, [bps] ; 8 bits or 16 bits
	shr	al, 4 ; 8 -> 0, 16 -> 1
	shl	al, 1 ; 16 -> 2, 8 -> 0
	mov	bl, [stmo] ; 2 = stereo, 1 = mono
	dec	bl
	or	bl, al
...
ScopeLoop:
	...
	; 24/06/2017
	mov	ecx, DMA_Buffer + (65536 - (320*4))
	cmp	esi, ecx 
	jna	short _4
	mov	esi, ecx
_4:
...
ch_silence:
	...
	;mov	cx, [numtracks] ; 18/10/2017
	mov	cl, [numtracks] ; 19/10/2017
...
_8_channels_mix:
	...
	add	ah, dh ; ch1 + ch4 + ch5 + ch8
	add	al, dl ; ch2 + ch3 + ch6 + ch7
...
c_smpl_1:
	...
	stosw	; save 16 bit left channel data ; 17/10/2017

	; 17/10/2017 (Right channel)
	lodsb	; get 8 bit mono sample
	mov	ah, al
	sub	ah, 80h
	xor	al, al
	;
	stosw	; save 16 bit right channel data
