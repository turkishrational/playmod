...
_4:	 
	...
	; 03/08/2020
        ;; load 65536 bytes into audio buffer
	;mov	edi, Audio_Buffer
	;mov	ebx, BUFFERSIZE ; 65536 bytes ; 01/08/2020
	call	GetSamples
	jc	short error_exit
	...
	;; load 65536 bytes into audio buffer
	;mov	edi, Audio_Buffer
	;mov	ebx, BUFFERSIZE ; 65536 bytes
	call	GetSamples
	jc	short error_exit

	; 09/10/2017 (2*BUFFERSIZE, 64K)
	; 23/06/2017 (2*65536, 128K)
	...
	;; 13/10/2017
	;mov	edi, Audio_Buffer
	;mov	ebx, BUFFERSIZE ; 32768 bytes ; 09/10/2017
	;call	GetSamples
...
_a3:
	...
	int	31h
	; dh = row, dl = column
	; 06/10/2017 (scroll check on next line)
	cmp	dh, 24 ; the last row
	jb	short set_bar_pos
	dec	dh
set_bar_pos:
	mov	al, 80
	mul	dh
...
ac97_int_handler:
	...
	;; 14/10/2017
	;mov	edi, Audio_Buffer
	;mov	ebx, BUFFERSIZE ; 32768 bytes
	call	GetSamples
	;jc	error_exit ; 30/07/2020
...
BarLoop:
	...
	; 02/10/2017 
	mov     ecx, [bar_stop]	; get previous bar stop address
	mov	edi, [bar_start]
	; 06/10/2017
	mov	ax, 0700h ; Blank
	mov	[edi], ax
	sub	ecx, edi
	jna	short check_volume
	mov	edx, edi
	shr	cl, 1
	rep	stosw
	mov	edi, edx
	mov	edx, [prev_max]
	or	edx, edx
	jz	short check_volume
	cmp	edx, edi
	jna	short check_volume
	mov	al, 0DBh ; 219 ; Block
...
draw_bar:
	...
	dec	edx	
	cmp	edx, [prev_max]
	jne	short new_max_volume
	mov	[prev_max], ecx ; 0 ; *
	;jmp	short blank_prev_max
	jmp	short update_bar_stop
new_max_volume:	 
	mov	ah, 0Ch ; Light Red
	mov	[edx], ax
	xchg	[prev_max], edx
	;cmp	edx, [prev_max] ; ** not necessary ?!
	;ja	short update_bar_stop ; **  not necessary ?!
blank_prev_max:
	mov	[edx], ecx ; 0 ; *
update_bar_stop:
	mov	[bar_stop], edi
...
;MixSpeed:	dw 11025 ; Hz ; 30/07/2020
...
; MODPLAY.ASM
PitchTable:	resw 857 ; 23/08/2020
		;resw 3425 ; 01/10/2017 (TMODPLAY.ASM)
		resw 3 ; 23/08/2020
VolTable:	resb 16640
MixBuffer       resb MixBufSize ; 8192
...
Audio_Buffer:
		resb BUFFERSIZE ; 32768 ; 23/08/2020

alignb 65536

DMA_Buffer:	resb 2*BUFFERSIZE  ; 65536 ; 23/08/2020 
