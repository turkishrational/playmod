; ****************************************************************************
; playmod4.s (for TRDOS 386)
; ----------------------------------------------------------------------------
; PLAYMOD4.PRG ! VIA VT8237R MOD PLAYER & VGA DEMO program by Erdogan TAN
;
; 05/03/2017
;
...
MakeOfs:        
	...
	sys	_audio, 0D00h, 2*65536, DMA_Buffer
	;jc	error_exit
...
modp_gs:
	mov     edi, Audio_Buffer
	mov	ebx, BUFFERSIZE ; 65536 bytes ; 25/06/2017
	call    GetSamples
q_loop:
...
DrawLoop:       
	...
	;lodsb
	;mov	bl, al
	mov	bl, [esi]	; get a sample (8-bit)
	inc	esi		; calc new pixel address...
	xor     bh, bh
	shl     bx, 1
	mov     di, [RowOfs+ebx]
	add     di, cx
...
msg_usage:
	...
	db	'usage: playmod filename.mod', 10,13,0
