; ****************************************************************************
; sb16mod.s (for TRDOS 386)
; ----------------------------------------------------------------------------
; SB16MOD.PRG ! SOUND BLASTER 16 MOD PLAYER & VGA DEMO program by Erdogan TAN
;
; 20/10/2017  !!! STEREO MOD PLAYING !!!
;
; [ Last Modification: 21/10/2017 ]
;
; Stereophonic mod playing code prototype: 
;		'modplay6.s' (AC97) by Erdogan Tan (18/10/2017
;
...
; Assembler: NASM 2.11
; ----------------------------------------------------------------------------
;	   nasm  sb16mod.s -l sb16mod.txt -o SB16MOD.PRG	
; ****************************************************************************
...
GetDmaCount:
		; 20/10/2017
		;in	al, 0C6h
		mov	dx, 0C6h ; 16 bit DMA, channel 5, count
		;mov	ah, 0 ; inb
		;int	34h	
		;mov	cl, al
		;in	al, 0C6h
		...
		;int	34h
		;mov	ch, al
		mov	ah, 6 ; inb & inb ; 12/10/2017 (TRDOS 386, int 34h)
		int	34h	
		mov	cx, ax
		; 20/10/2017
		inc	cx ; word count -1 to word count
		shl	cx, 1 ; word to byte
		; 06/10/2017
		;mov	dx, 65535
		;mov	edx, DmaBufSize - 1
		mov	edx, DmaBufSize ; 20/10/2017
		;mov	esi, DmaBufSize - 1  ; <= 65535
		;sub	dx, cx
		sub	edx, ecx ; 21/10/2017
		;sub	si, cx
		; 15/03/2017
		;sub	dx, 160
		; 21/10/2017
		;sub	edx, 160*4
		sub	dx, 160*4 ; 16 bit, stereo
		; set to Left Channel, 1st Byte
		and	edx, 0FFFCh ; clear bit 0 & 1
;sss_offset_loop:
;		cmp	dx, DmaBufSize
;		jb	short sss_offset
;		sub	dx, DmaBufSize
;		jmp	short sss_offset_loop
;sss_offset:
		;add	si, dx
...
Success:        
		mov     dx, [SbAddr]		; Print Successful message.
		mov     cl, [SbIrq]
		shr     dl, 4
		add     dl, '0'
		mov     [PortText], dl
...
SbIrqHandler:	; SoundBlaster IRQ Callback service for TRDOS 386
		...
		mov     dx, [SbAddr]
		;add	dx, 0Eh ; DSP interrupt acknowledge (8 bit)
		; 20/10/2017
		add	dx, 0Fh	; DSP interrupt acknowledge (16 bit)
		;in	al, dx
		;mov	ah, 0
		sub	ah, ah
		int	34h
		...
		;sub	dx, 02h ; 0Ch ; 0Eh -> 0Ch (8 bit transfer)
		;sub	dx, 03h ; 0Fh -> OCh (16 bit transfer)

		; exit auto-initialize 16 bit transfer
		;SbOut	0D9h ; exit auto-initialize transfer command
		;jmp	short a_5
a_3:
		xor	al, al ; 0
		mov	[iStatus], al
...
SecondHalf:
		add     edi, ebx
FirstHalf:
		; 20/10/2017 (16 bit, stereo soud data)
		shr	bx, 2 ; DmaBufSize/8 ; 8192
		call    GetSamples
Bye:
		popad
ByeBye:
...
;		pushad
;		
;		mov     dx, 03h  ; 8 bit dma, channel 1 count
;		mov	dx, 0C6h ; 16 bit dma, channel 5 count
;		;in	al, dx
;		;mov	ah, 0
;		sub	ah, ah
...
SbOk:
		...
		add	dx, 0Ch-05h
StartDma:  
		;; autoinitialized 16 bit DMA transfer mode
		;mov	bx, 30B6h ; 16 bit, stereo output
		; PCM output (16 bit stereo autoinitialized transfer)
		;SbOut  bl ; bCommand
		;SbOut	bh ; bMode
		SbOut	0B6h ; bCommand (B6h = 16 bit autoinitialized)
		SbOut	30h  ; bMode (30h = 16 bit stereo)	
		;mov	bx, DmaBufSize / 2
		mov	bx, DmaBufSize / 4 ; 20/10/2017
...
GetSamples:	; mixpoll ; 01/10/2017 (TMODPLAY.ASM)
	...
	; 20/10/2017
	cmp	byte [pan_shift], 0
	jna	short c_smpl_0

	mov	[audio_buf_size], ebx ; DmaBufSize/8 ; 8192
...
channel_mix_ok:
	...
	; 20/10/2017
	mov	ebx, [audio_buf_size] ; DmaBufSize*8 ; 8192
...
msg_usage:
		...
		db	'usage: sb16mod filename.mod', 10, 13,0
		db	'21/10/2017',0
...
sampling_rate:
MixSpeed:	dw 22050 ; Hz ; 20/10/2017
;MixSpeed:	dw 22222 ; Hz 
...
VolTable:	resb 16640
MixBuffer       resb 8172 ; MixBufSize ; 7680 (960*8) ; 20/10/2017
...
; 20/10/2017
audio_buffer:	resd 1 ; = DmaBuffer or DmaBuffer + DmaBufSize/2
audio_buf_size:	resd 1 ; = DmaBufSize/8 ; 8192
